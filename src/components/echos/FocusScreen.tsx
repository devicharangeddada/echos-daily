        ...
        <motion.div {...fadeInUp} className="glass-card p-4 mb-6 rounded-[2rem] bg-black/40 border border-white/10">
          <div className="flex items-center justify-between">
            ...
            <div className="flex gap-2">
              <button
                onClick={() => setHum((v) => !v)}
                className={`rounded-full px-3 py-1 text-xs ${hum ? 'bg-accent text-black' : 'bg-white/10'}`}
              >Transformer Hum</button>
              <button
                onClick={() => setWhiteNoise((v) => !v)}
                className={`rounded-full px-3 py-1 text-xs ${whiteNoise ? 'bg-accent text-black' : 'bg-white/10'}`}
              >White Noise</button>
            </div>
          </div>
        </motion.div>
